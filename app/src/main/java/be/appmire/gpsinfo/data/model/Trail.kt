...
data class Trail(
    ...
    /**
     * Target pace the runner set for this trail, normalised to seconds
     * per kilometre (the display layer converts to whatever unit the
     * user has selected). Persisted to GPX under a private GPSinfo
     * extension namespace; null when no target was active during the
     * recording. Drives the post-run performance score (#48).
     */
    val targetPaceSecondsPerKm: Float? = null,
    /**
     * Lap splits inserted during the recording. Empty when the runner
     * never tapped the lap button. Persisted in GPX under the private
     * GPSinfo extension namespace.
     */
    val laps: List<LapMarker> = emptyList(),
    /**
     * Free-form user tags. Persisted in GPX `<metadata><keywords>`
     * as a comma-separated list — the standard GPX 1.1 slot for
     * categorisation. Stable identity is the literal string;
     * comparison is case-sensitive (intentional — "Run" and "run"
     * are different tags if the user typed them that way).
     */
    ...
    /** All non-null BPM samples captured during the recording. Empty when
     *  the trail predates BLE HR pairing or the user never paired a
     *  monitor. Computed lazily; callers wanting both [avgHr] and
     *  [maxHr] should cache the result themselves. */
    val hrSamples: List<Int> get() = points.mapNotNull { it.heartRateBpm }

    /** Mean BPM across the trail. Null when no HR samples are present. */
    val avgHr: Int? get() {
        val s = hrSamples
        return if (s.isEmpty()) null else (s.sum() / s.size)
    }

    /** Peak BPM across the trail. Null when no HR samples are present. */
    val maxHr: Int? get() = hrSamples.maxOrNull()

    /**
     * Seconds spent in each HR zone (Z1..Z5 → indices 0..4) under the
     * given zone configuration. Attributes the time between consecutive
     * points to the zone of the *earlier* point's HR, which is the
     * standard convention used by Garmin / Strava / TrainingPeaks.
     * Returns all-zero array when the trail has no HR samples.
     */
    fun timeInZoneSeconds(config: HrZoneConfig): IntArray {
        val out = IntArray(5)
        for (i in 1 until points.size) {
            val prev = points[i - 1]
            val cur = points[i]
            val bpm = prev.heartRateBpm ?: continue
            val dt = (cur.timeMillis - prev.timeMillis) / 1000L
            if (dt <= 0) continue
            val zone = config.zoneFor(bpm).coerceIn(1, 5)
            out[zone - 1] += dt.toInt()
        }
        return out
    }
